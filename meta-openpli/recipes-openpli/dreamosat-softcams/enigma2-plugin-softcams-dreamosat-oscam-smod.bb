...
PACKAGES = "enigma2-plugin-softcams-dreamosat-oscam-smod"
PROVIDES = "${PACKAGES}"
...
SRC_URI = "git://github.com/oe-alliance/oscam-smod;protocol=https;branch=master \
...
do_install() {
	...
	install -d ${D}${bindir}
	install -m 0755 ${B}/oscam ${D}${bindir}/${CAMNAME}_${REVISION}-r${EMUVERSION}
}
...
if [ -f /etc/apt/apt.conf ] ; then
    if [ -s /usr/lib/dpkg/info/enigma2-plugin-softcams-dreamosat-oscam-smod.list ] ||
    [ -s /var/lib/dpkg/info/enigma2-plugin-softcams-dreamosat-oscam-smod.list ]; then
    ...
    if [ -s /usr/lib/opkg/info/enigma2-plugin-softcams-dreamosat-oscam-smod.list ] ||
    [ -s /var/lib/opkg/info/enigma2-plugin-softcams-dreamosat-oscam-smod.list ]; then
        echo "DreamOSat Softcams ${CAMNAME} exist. No freesize check"
    else
        if [ 3484 -gt "${FREEsize}" ]; then
            echo -e "Paketsize 3484kb tis to big for your Flashsize ${FREEsize}kb"
