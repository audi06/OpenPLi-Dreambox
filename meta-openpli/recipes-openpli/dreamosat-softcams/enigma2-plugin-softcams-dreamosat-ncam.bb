DESCRIPTION = "DreamOSat camManager for ${CAMNAME} svn-${REVISION} emu r${EMUVERSION} Open Source Softcam"
SUMMARY = "${CAMNAME} - Open Source Softcam - svn-${REVISION} emu r${EMUVERSION}"
...
"
LICENSE = "CLOSED"
PACKAGES = "enigma2-plugin-softcams-dreamosat-ncam"
PROVIDES = "${PACKAGES}"
...
SRC_URI = "git://github.com/fairbird/NCam.git;protocol=https;branch=master \
    file://${CONF_FILES}/AutoRoll.Key \
    file://${CONF_FILES}/CCcam.cfg \
    file://${CONF_FILES}/constant.cw \
    file://${CONF_FILES}/ncam.ccache \
    file://${CONF_FILES}/ncam.conf \
    file://${CONF_FILES}/ncam.dvbapi \
    file://${CONF_FILES}/ncam.fs \
    file://${CONF_FILES}/ncam.provid \
    file://${CONF_FILES}/ncam.server \
    file://${CONF_FILES}/ncam.services \
    file://${CONF_FILES}/ncam.srvid2 \
    file://${CONF_FILES}/ncam.user \
    file://${CONF_FILES2}/SoftCam.Key \
    "
...
CONF_FILES = "/home/dreamosat/Genel/Emulator/EMU_BINAR/config/ncam/etc/tuxbox/config"
CONF_FILES2 = "/home/dreamosat/GitHub/DreamOSatScript/CaMs-EmU"

CONFFILES = "\
	${sysconfdir}/tuxbox/config/AutoRoll.Key \
	${sysconfdir}/tuxbox/config/CCcam.cfg \
	${sysconfdir}/tuxbox/config/constant.cw \
	${sysconfdir}/tuxbox/config/ncam.ccache \
	${sysconfdir}/tuxbox/config/ncam.conf \
	${sysconfdir}/tuxbox/config/ncam.dvbapi \
	${sysconfdir}/tuxbox/config/ncam.fs \
	${sysconfdir}/tuxbox/config/ncam.provid \
	${sysconfdir}/tuxbox/config/ncam.server \
	${sysconfdir}/tuxbox/config/ncam.services \
	${sysconfdir}/tuxbox/config/ncam.srvid2 \
	${sysconfdir}/tuxbox/config/ncam.user \
	${sysconfdir}/tuxbox/config/SoftCam.Key \
	"

FILES:${PN} = "\
	${bindir}/${CAMNAME}_${REVISION}-r${EMUVERSION} \
	${sysconfdir}/tuxbox/config/* \
	${prefix}/camscript/${CAMNAME}_${REVISION}-r${EMUVERSION}.sh \
	"
...
do_compile:prepend() {
	...
	then
		echo "${revision}"
		export EMUVERSION="test"
	else
		echo "${revision}-r${emuversion}"
		export EMUVERSION="${emuversion}"
	fi
}
...
do_install:append () {
cat > ${S}/${CAMNAME}_${REVISION}-r${EMUVERSION}.sh << EOF
#!/bin/sh
#### "*******************************************";
#### "*          ..:: A U T H O R ::..          *";
#### "*             << Audi06_19 >>             *";
...
OSD="${CAMNAME}_${REVISION}-r${EMUVERSION}"
CAM="${CAMNAME}_${REVISION}-r${EMUVERSION}"
PID=\`pidof \${CAM}\`
Action=\$1
...
do_install() {
	...
	install -d ${D}${bindir}
	install -m 0755 ${B}/ncam ${D}${bindir}/${CAMNAME}_${REVISION}-r${EMUVERSION}
}

# Install routine, should be ok for most cams.
do_install:append () {
	install -d ${D}${prefix}/camscript
	install -m 755 ${S}/${CAMNAME}_${REVISION}-r${EMUVERSION}.sh ${D}${prefix}/camscript/${CAMNAME}_${REVISION}-r${EMUVERSION}.sh
}
...
if [ "x$D" == "x" ]; then
    rm -rf ${bindir}/${CAMNAME}_${REVISION}-r${EMUVERSION} > /dev/null 2>&1
    rm -rf ${prefix}/camscript/${CAMNAME}_${REVISION}-r${EMUVERSION}.sh > /dev/null 2>&1
fi
exit 0
}
...
if [ -f /etc/apt/apt.conf ] ; then
    if [ -s /usr/lib/dpkg/info/enigma2-plugin-softcams-dreamosat-ncam.list ] ||
    [ -s /var/lib/dpkg/info/enigma2-plugin-softcams-dreamosat-ncam.list ]; then
    ...
    if [ -s /usr/lib/opkg/info/enigma2-plugin-softcams-dreamosat-ncam.list ] ||
    [ -s /var/lib/opkg/info/enigma2-plugin-softcams-dreamosat-ncam.list ]; then
        echo "DreamOSat Softcams ${CAMNAME} exist. No freesize check"
    else
        if [ 3484 -gt "${FREEsize}" ]; then
            echo -e "Paketsize 3484kb tis to big for your Flashsize ${FREEsize}kb"
